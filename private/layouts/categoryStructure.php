<?php
    ...
    if (isset($Categories_array[$parent]) && isset($Categories_array[$sub])) {
        // parent layer 
        foreach ($Categories_array[$parent] as $Category) {
            // escape potential html characters
            $value = h($Category->title);
            echo "<a href='add_edit_category?categoryId={$Category->get_id()}'>{$value}</a><br>";
